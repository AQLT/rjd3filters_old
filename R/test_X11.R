...
mod <- RJDemetra::get_jmodel(wk)[[1]][[1]]
extract <- function(id) {
...
compare( x = M3X3_s * (si_1 * y), "b5")


AQLTools::hc_stocks(ts.union(s_1 * y, extract("b5")))

mm_s_norm = finite_filters((1 - M2X12), rfilters = lapply(1:6, function(i) (1 - M2X12)*moving_average(1, lags = -i) #* moving_average(1, lags = -12)
))
...
M3X5_s <- rjd3filters:::finite_filters(moving_average(M3X5[,4],-3),
                                       list(moving_average(M3X5[,1], -3),
                                            moving_average(M3X5[,2], -3),
                                            moving_average(M3X5[,3], -3)),
                                       first_to_last = T)
M3X5_s = to_seasonal(M3X5_s, 12)
s_2 = M3X5_s * si_2
dim(as.matrix(si_2))
...
rjd3modelling::modelling_context()
